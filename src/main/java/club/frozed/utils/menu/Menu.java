...
public abstract class Menu {
	...
	public void openMenu(final Player player) {
		...
		if (player.getOpenInventory() != null) {
			if (previousMenu == null) {
				...
				if (this instanceof PaginatedMenu) {
					...
					update = true;
				} else if (previousSize == size && player.getOpenInventory().getTopInventory().getTitle().equals(title)) {
					inventory = player.getOpenInventory().getTopInventory();
					update = true;
				} else {
		...
		if (autoUpdate && task == null) {
			task = FrozedSmashClub.getInstance().getServer().getScheduler().runTaskTimer(FrozedSmashClub.getInstance(), () -> this.openMenu(player), 0, 20L);
		}
	}
