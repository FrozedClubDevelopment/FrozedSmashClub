package club.frozed.utils.menu;

import club.frozed.FrozedSmashClub;
import club.frozed.utils.menu.pagination.PaginatedMenu;
...
public class ButtonListener implements Listener {
		...
		if (openMenu != null) {
			...
			if (openMenu.getButtons().containsKey(event.getSlot())) {
				...
				if (event.isCancelled()) {
					Bukkit.getScheduler().runTaskLater(FrozedSmashClub.getInstance(), () -> player.updateInventory(), 1L);
				}
			} else {
	...
	public void onInventoryClose(InventoryCloseEvent event) {
		...
		player.setMetadata("scanglitch", new FixedMetadataValue(FrozedSmashClub.getInstance(), true));
	}
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		...
		if (player.hasMetadata("scanglitch")) {
			player.removeMetadata("scanglitch", FrozedSmashClub.getInstance());
